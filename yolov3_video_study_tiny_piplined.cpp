    ...
    int input_width)
{
    while (true)
    {
        ...
        Mat img = postprocess(
            ...
            input_height,
            input_width);
        out.push(make_pair(dpuOutput.index, img));
    }
}
...
int main(const int argc, const char **argv)
{
    ...
    concurrent_queue<DpuInputFrame> dpuIn(30);
    concurrent_queue<DpuOutputFrame> dpuOut(30);
    array<thread, 8> threadsList = {
        ...
        thread(runDPU, runner1.get(), ref(dpuIn), ref(dpuOut)),
        thread(runDPU, runner2.get(), ref(dpuIn), ref(dpuOut)),
        thread(runDPU, runner3.get(), ref(dpuIn), ref(dpuOut)),
